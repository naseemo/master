	...
	<body class="smoothscroll enable-animation">
		...
		@include('header')
				...
				<div class="step1">
					<div class="row margin-bottom-30">
						...
						<label class="size-16">Item Condition <span class="text-danger">*</span></label>
							...
							<label class="radio" style="font-weight: normal;">
								<input type="radio" name="item_condition" value="Used" data-ng-model="item_condition" checked="checked" required="" {{ old('item_condition')=='Used'?'checked':'1' }}>
								<i></i> Used
							</label>
							<label class="radio" style="font-weight: normal;">
								<input type="radio" name="item_condition" value="New" data-ng-model="item_condition" required="" {{ old('item_condition')=='New'?'checked':'1' }}>
								<i></i> New
							</label>
						</div>
					</div>
					<div class="row margin-bottom-30">
						...
						<label class="size-16">Ad Title <span class="text-danger">*</span></label>
							<input type="text" class="form-control size-16 font-bold" placeholder="Enter your ad title here" data-ng-model="title" required="" name="title" value="{{ old('title') }}"/>
						</div>
					</div>
					<div class="row">
							...
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									@foreach($categories as  $cat)
										...
										<button type="button" class="btn btn-default catbutton catbtn_{{$cat->id}})" onclick="catbutton({{$cat->id}})"><i class="{{$cat->subc_desciptions}}"></i> {{$cat->subc_name}}</button>
										</div>
									@endforeach
									</div>
					...
					<div class="step2" style="display:none;">
						<div class="row">
							...
							<label class="size-16">Ad Description <span class="text-danger">*</span><br/><small style="font-weight: normal;">Enter details of your item</small></label>
								<!-- textarea -->
								<div class="fancy-form" data-ng-init="">
									<textarea class="form-control word-count size-16" name="ad_desc" id="ad_desc" placeholder="Write your ad details here..." required="" maxlength="5000" data-ng-maxlength="5000" data-info="textarea-words-info" data-ng-model="ad_desc" style="min-height:250px;">{{ old('ad_desc') }}</textarea>
						...
						<div class="row">
							...
							<label class="size-16">Item Price <span class="text-danger">*</span></label>
								...
								<div class="fancy-form">
									...
									<input type="text" class="form-control size-16 font-bold" name="item_price" id="item_price" required="" data-ng-model="item_price" style="padding-left: 40px;" value="{{ old('item_price') }}">
						...
						<div class="row">
							...
							<div class="panel-body">
								...
								<div class="fancy-form"><!-- input -->
									...
									<input type="text" class="form-control" name="fname" id="fname" placeholder="Your first name" value="{{ old('fname') }}">
									<span class="fancy-tooltip top-left"> <!-- positions: .top-left | .top-right -->
								...
								<div class="fancy-form"><!-- input -->
									<i class="fa fa-user"></i>
									<input type="text" class="form-control" name="lname" id="lname" placeholder="Your last name" required="" value="{{ old('lname') }}">
									<span class="fancy-tooltip top-left"> <!-- positions: .top-left | .top-right -->
								...
								<div class="fancy-form"><!-- input -->
									...
									<input type="text" class="form-control masked" name="phone" id="phone" data-format="(+999) 999-999999" data-placeholder="X" placeholder="Enter telephone" required="" value="{{ old('phone') }}">
								...
								<div class="fancy-form"><!-- input -->
									...
									<input type="email" class="form-control valid" name="email" id="email" placeholder="Enter your email" required="" value="{{ old('email') }}">
									<span class="fancy-tooltip top-left"> <!-- positions: .top-left | .top-right -->
								...
								<div class="fancy-form"><!-- input -->
									...
									<input type="text" class="form-control" name="address" id="address" placeholder="Enter your address" required="" value="{{ old('address') }}">
									<span class="fancy-tooltip top-left"> <!-- positions: .top-left | .top-right -->
								...
								<select class="form-control " style="width:100%;" name="city" id="city" data-ng-model="city" required="" >
									...
									<option value="{{$loc->lc_name}}" {{ old('city') == $loc->id ? 'selected' : '' }}>{{$loc->lc_name}}</option>
									@endforeach
								</select>
								</div>
						...
						<div class="row margin-bottom-10">
							...
							<!-- radio -->
							<label class="radio" style="font-weight:normal;">
								<input type="radio" name="featured" id="featured" value="0" checked="checked" {{ old('featured')=='0'?'checked':'1' }}>
								<i></i> No, Thanks
							</label>
							</div>
							<div class="col-md-3">
							<label class="radio" style="font-weight:normal;">
								<input type="radio" name="featured" id="featured" value="1" {{ old('featured')=='1'?'checked':'1' }}>
								<i></i> Yes (AED 150)
							</label>
							</div>
						</div>	
						...
						<div class="col-md-12">
							<label class="checkbox nomargin"><input class="checked-agree" type="checkbox" name="checkbox_agree" required="" value="1" {{ old('checkbox_agree')=='on'?'checked':'1' }}><i></i>I agree to the <a href="#" data-toggle="modal" data-target="#termsModal">Terms of Service</a></label>
							</div>
							<div class="col-md-12">
							<label class="checkbox nomargin"><input type="checkbox" name="nl" value ="1" {{ old('nl')=='on'?'checked':'1' }}><i></i>I want to receive news and  special offers</label>
							</div>
						</div>
			...
			<!-- /TERMS & CONDITIONS -->
				...
				<h3 class="nomargin nopadding ng-binding size-19" data-ng-bind="title">title </h3>
				<div class="col-md-12 clearboth adlocationbar size-12" style="background:none; border:0;">
					<div class="col-md-12 nopadding">
					<i class="fa fa-map-marker nopadding" data-ng-bind="city"></i>  <i class="fa fa-calendar"></i> {{ date('Y-M-d') }} <i class="fa fa-shopping-cart" data-ng-bind="item_condition"></i>  item
					</div>
				</div>
				...
				<div class="col-md-12 clearboth adlocationbar size-12" style="background:none; border:0;">
					...
					AED   <p data-ng-bind="item_price"></p> 
					</div>
				</div>
				<p class="clearboth size-13 font-bold nomargin">Ad Details</p>
				<p class="clearboth size-13" style="margin-top:5px; text-align:justify;" data-ng-bind="ad_desc"> </p>
				</div>
				</div>
...
$('.level_1').html('<span style="padding-left:10px;"><img src="{{ asset("images/loaders/9.gif") }}" /> Please wait...</span>');
...
var cat_id = id;
 ...
 type: 'POST',
 url: "{{ url('/getCategory') }}",
 data:{cat_id:cat_id,_token: token,"_method": 'POST',level:0},
...
$('.level_2').html('<span style="padding-left:10px;"><img src="{{ asset("images/loaders/9.gif") }}" /> Please wait...</span>');
$.ajaxSetup({
			headers: 	{
			'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
						}
...
var cat_id = id;
 ...
 type: 'POST',
 url: "{{ url('/getCategory') }}",
 data:{cat_id:cat_id,_token: token,"_method": 'POST',level:1},
 success: function (response) {
	 if(response == 0){
...
function level_2(id){
//alert(id);
$('.level_3').html('<span style="padding-left:10px;"><img src="{{ asset("images/loaders/9.gif") }}" /> Please wait...</span>');
$.ajaxSetup({
			headers: 	{
			'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
						}
...
var cat_id = id;
 ...
 type: 'POST',
 url: "{{ url('/getCategory') }}",
 data:{cat_id:cat_id,_token: token,"_method": 'POST',level:2},
 success: function (response) {
	 if(response == '0'){
...
function level_3(id){
//alert(id);
$('.level_4').html('<span style="padding-left:10px;"><img src="{{ asset("images/loaders/9.gif") }}" /> Please wait...</span>');
$.ajaxSetup({
			headers: 	{
			'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
						}
...
var cat_id = id;
 ...
 type: 'POST',
 url: "{{ url('/getCategory') }}",
 data:{cat_id:cat_id,_token: token,"_method": 'POST',level:3},
 success: function (response) {
	 if(response == '0'){
