			...
			<section class="nopadding" style="background: url(assets/images/backgrounds/bg1-min.jpg);">
				...
				<div class="col-md-8 padding-top-50">
					...
					@if(Session::has('REG-MSG'))
									...
									@endif
					<form class="nomargin sky-form" action="{{URL::to('ticketing')}}" method="post">
					...
					<div style="clear:both;"></div>
					<p class="nomargin nopadding size-13 text-left"><i class="fa fa-info-o"></i> <strong>Notice:</strong> To get lucky coupon number you need to <a href="{{URL::to('membership')}}" class="btn btn-xs"><i class="fa fa-plus-circle fa-lg"></i> Post a Featured Ad</a><p>
				</div>
				<div class="col-md-4">
				...
				@if(count($winnersArr)>0)
					...
					<span class="block"><a href="public_profile.php" class="text-default"><i class="fa fa-user"></i> {{$winnersArr->firstname}} {{$winnersArr->lastname}}</a></span>
					<small class="text-default font-weight-normal size-13"><i class="fa fa-ticket"></i>{{$winnersArr->ticket_no}}</small>
					<small class="text-red size-13"><i class="fa fa-trophy"></i>{{$winnersArr->ticket_price}}</small>
					</div>
					@endforeach
					@else
						<small class="block size-16 margin-bottom-30">NO WINNERS</small>
					@endif
				</div>
				</div>
			</section>
